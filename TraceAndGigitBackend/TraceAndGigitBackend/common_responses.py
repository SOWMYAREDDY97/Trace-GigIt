...
MODULE_NAME = "myplex_service.apps.common.responses"
...
class BillingResult(Result):
    def __init__(self,code,status,message,pack_type='',web_based=False,extra_fields=None):
        """
        Base Result class for Billing API responses.
        code - numeric status code, similar to HTTP status code
        status - short status code
        message - descriptive message.
        web_based - to know weather to redirect or direct http response as json
        """
        super(BillingResult,self).__init__(code,status,message,extra_fields)
        self.web_based = web_based
        self.pack_type = pack_type
    def http_response(self, pretty=0, status_code=200):
        if self.web_based and self.pack_type.lower() != 'svod':
            return HttpResponse(self.extra_fields.get('redirectLink'),mimetype='text/html')
        elif self.web_based:
            redirectLink = "http://myplexnow.tv/?status=%s&action=%s&message=%s"%(self.status,'showMessage',self.message)
            if self.code == 200:
                redirectLink = self.extra_fields.get('redirectLink')
            LOGGER.info("Billing redirect link is :%s",redirectLink)
